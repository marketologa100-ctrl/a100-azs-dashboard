...
def convert_to_xlsx(xls_path: Path, tmpdir: Path) -> Path:
    """Конвертирует .xls в .xlsx. Использует xlrd если файл читается, иначе pandas."""
    import xlrd
        ...
        for sheet_name in wb_in.sheet_names():
            ...
            for row_idx in range(ws_in.nrows):
                for col_idx in range(ws_in.ncols):
                    ...
                    ws_out.cell(row=row_idx+1, column=col_idx+1, value=cell.value)
        wb_out.save(str(out))
    except Exception:
        # Fallback: читаем напрямую через pandas xlrd engine и сохраняем как xlsx
        ...
        for sname in xl.sheet_names:
            ...
            for r_idx, row in enumerate(df.itertuples(index=False), 1):
                for c_idx, val in enumerate(row, 1):
                    ws_out.cell(row=r_idx, column=c_idx, value=val if str(val) != 'nan' else None)
        wb_out.save(str(out))
    return out
